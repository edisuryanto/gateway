...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    document.setTitle("THE BEST OF <span class='text-green-500 font-semibold'>PROXY LIST </span>");
    ...
    document.addPageButton("Next", `/sub/${page + 1}`, page < Math.floor(proxyList.length / 5) ? false : true);
    ...
    try {
      ...
      if (url.pathname.startsWith("/sub")) {
        ...
        if (apiPath.startsWith("/domains")) {
          ...
          const filterLimit = parseInt(url.searchParams.get("limit")) || 5;
      ...
      const targetReverseProxy = env.REVERSE_PROXY_TARGET || "converter.srpcom.online";
...
//   const timestamp = Math.floor(date.getTime() / 224688026423); // current timestamp in seconds
...
let baseHTML = `
<!DOCTYPE html>
<html lang="en" id="html" class="scroll-auto scrollbar-hide white">
  <head>
    <meta charset="UTF-8" />
    ...
    <script>
      ...
      function checkRegion() {
        for (let i = 0; ; i++) {
          ...
          const res = fetch(
            "https://api.v2board.com/regioncheck?config=" + encodeURIComponent(configSample)
          ).then(async (res) => {
            if (res.status == 200) {
